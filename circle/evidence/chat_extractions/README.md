...
| `DIG_COMPOUND_SWEEP_08_belief_justification_action_and_belief_rescue.md` | ⚡ Compound sweep: subjective experience as evidence (E1–E4 split); knowledge vs action thresholds; **what defeating a reason actually defeats** (only argument-level defeat is automatic); **belief rescue vs immunization** — Ziggy's admitted rescue impulse is the hottest prey; ⚠️ opens on Nova's reconstruction of an UNAVAILABLE earlier conversation | 🟡 IN PROGRESS — Stage 1 ⚠️ conditional (source-boundary overlay: G4/G5 knocked down) · Stage 2 ✅ reviewed; **Stage 3 authorized**; forecast scored 6/6; 12 moves → 6 families; **Belief Rescue = Z-DIRECT self-observation, high-value seed (recurrence unconfirmed)**; rebutting/undercutting defeater split ordered; selective-bridge-inspection failure mode named; *self-exposure → symmetry restoration* = strongest interaction finding; 5 trails at close (IT-045…049) |
