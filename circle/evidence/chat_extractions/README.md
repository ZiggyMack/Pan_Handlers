...
| `DIG_COMPOUND_SWEEP_08_belief_justification_action_and_belief_rescue.md` | ⚡ Compound sweep: subjective experience as evidence (E1–E4 split); knowledge vs action thresholds; **what defeating a reason actually defeats** (only argument-level defeat is automatic); **belief rescue vs immunization** — Ziggy's admitted rescue impulse is the hottest prey; ⚠️ opens on Nova's reconstruction of an UNAVAILABLE earlier conversation | ✅ **COMPLETE (review-complete) — candidate harvest, ZERO promotions (plow-through held, 7th consecutive)**; ⚠️ **Stage-4 blind output still owed to the workbook**; 4 durable families; **Belief Rescue ≠ Possibility Preservation**; repair/immunization discriminator (*may the replacement reasons fail?*); **first confirmed relationship-level Reconstruction Bypass**; preservation bias = deepest structural finding; seeds IT-045…051 (7 trails); Grant: G-REPORTED/NOVA-RECONSTRUCTED NON-SITE |
