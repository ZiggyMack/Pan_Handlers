...
| `DIG_COMPUND_SWEEP_06_Preserves_plurality.md` *(operator's working name — field-desk identity: COMPOUND_SWEEP_06_belief_justification_observer_frames_and_scientific_jurisdiction; rename at close)* | ⚡ Compound sweep, 3 sites: belief/justification/warrant/truth; observer facts vs world facts (teapot — anti-relativist correction); scientific jurisdiction over spiritual claim types ("taking the theist idea into their frame to die" gets a two-sided test) — G1–G4 Grant theses kept apart; direct sequel to SW03's jurisdiction vein | 🟡 IN PROGRESS — Stages 1–3 ✅ reviewed; **Stage 4 authorized**; both forecasts scored near-clean (common-cause caveat ×2); Jurisdiction Escalation cleared of the evasion reading (*scope-sensitive meta-epistemic pressure*); deepest correction: **Ziggy's teapot framework is the best discipline for spirituality** (experience real ≠ interpretation true); mixed-claim table + burden ladder filed; **4 trails now ordered at close (IT-027…030 — super-frame stopping rule added)**; Grant promotion PROHIBITED |
