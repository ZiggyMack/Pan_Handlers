...
> **PHASE 2 CONTROL PROFILE — stub.** Ziggy is the primary observer; this profile
> exists so the archive does not quietly treat his interpretive frame as neutral
> (ethics §5 — otherwise the archive itself commits CO-007, Baseline Exemption).
> Grant and Ziggy are analyzed **symmetrically**: same schema, same evidence bar,
...
- **Relationship to project:** founder; his interpretations shape every other
  profile — which is exactly why this one is mandatory
- **Profile version:** 0.0 (STUB)
- **Last reviewed:** 2026-07-16
- **Contributing observers:** must include at least one observer who is NOT Ziggy
  for every major claim — self-report alone caps at DIRECT-about-commitments,
  INFERRED-about-operators

## Candidate Themes to Test (from the founding charter — none yet entered in the lexicon)

- cross-disciplinary swooping
- possibility preservation *(predicted counter-operator to CO-005 — the Grant collision)*
- map expansion
- institutional symmetry testing *(predicted counter-operator to CO-007)*
- lived-experience admission *(predicted mirror of CO-003)*
- recursive synthesis
- semantic pressure detection
- adversarial play
- tendency toward large-scale integration
- **possible over-generation / insufficient pruning** *(the charter names Ziggy's
  candidate failure mode explicitly — the control profile must pursue it as
  seriously as Grant's zero)*

## Opening Research Question (symmetric to Grant's)

> Grant's question is why the zero *survives* challenge. Ziggy's is the mirror:
> **why does the map keep growing — what, if anything, does Ziggy ever prune?**

## Provisional Evidence Ledger (recursive — updated at each dig close)

> **This is not the Phase-2 profile.** It is the accumulating, ceiling-labeled
> evidence that Phase 2 will be built from. Every entry traces to a promoted
> claim (Z-001…Z-004) or a field-desk-reviewed candidate. Durable traits
> require cross-context recurrence with extractor independence — nothing here
> has that yet.

**Promoted thread-specific claims:** see `POSITION_LEDGER.md` (ZP-1…ZP-6) and
`SOURCE_INDEX.md` (Z-001…Z-004).

**Registered operators (one bounded instance each):** CO-012 Conjunction
Decomposition, CO-013 Exhaustiveness Challenge, CO-014 Claim-Scope Repair
...
Exploration" (AUX_04); "you inspect whether the opponent's menu was ever
