# 🧠 Operator Lexicon — Shared Vocabulary of Cognitive Moves

> An **operator** is a recurring cognitive move used to reach, defend, revise, or
> reject beliefs — distinct from any position it produces. Operators are defined
> **here, once**, then instantiated per-member in `MEMBERS/*/OPERATOR_MAP.md` with
...
## Status Convention (aligned with the Nyquist Museum)

The Nyquist Cognitive Archaeology program maintains a Museum of Operators
(OP-001…019) extracted from historical thinkers. This lexicon is its **sibling
registry for the living circle**, and uses the same promotion ladder:

| Status | Meaning |
| -------- | --------- |
| 🔴 **RED** | Named candidate; definition exists; evidence pending or single-context |
| 🟡 **PROPOSED** | Evidence from one member across multiple discussions, or one clean documented instance |
| 🟢 **GREEN** | Validated: observed in ≥2 members or ≥3 independent debates, counterexample search performed |
| ⚫ **RETIRED** | Turned out to rename an existing operator, or dissolved under evidence |

**Cross-registry rule:** when a circle operator converges with a Museum operator,
record the OP-### link in its entry. Human-circle rediscovery of a Museum operator
...
- **Usual output:** what the move produces
- **Productive use:** where this move genuinely works
- **Failure mode:** where the same move misfires (Failure-Atlas principle:
  success and failure are the same move pointed at different targets)
- **Counter-operator:** the move that checks or reverses it
- **Museum link:** OP-### convergence, if any
- **Instantiated in:** member OPERATOR_MAP links
```
...
- **Usual output:** frameworks ranked by predictive yield; non-predictive virtues discounted.
- **Productive use:** empirical science, forecasting, model selection — domains where prediction is the native currency.
- **Failure mode:** exported to domains (metaphysics, norm generation, meaning systems) whose successes are not predictive in kind; see CO-002.
- **Counter-operator:** jurisdiction check — "what is this framework *for*, and what counts as success *there*?"
- **Museum link:** none yet.
...
- **Usual output:** the home-domain standard silently becomes the universal rubric.
- **Productive use:** legitimate unification — some standards *do* generalize; the move is only pathological when jurisdiction is asserted rather than argued.
- **Failure mode:** frameworks weak in the imported metric score near zero regardless of native-domain performance.
- **Counter-operator:** explicit jurisdiction argument before scoring.
- **Museum link:** none yet.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** the candidate reaches evaluation already deprived of standing.
- **Productive use:** filtering genuinely unreliable generators in high-stakes empirical contexts.
- **Failure mode:** the admission rule itself is never argued for; whole families of frameworks lose before playing.
- **Counter-operator:** generative authority audit — "who admitted the admitters?"
- **Museum link:** candidate relation to the Aesthetic Generation family (OP-016…019, PROPOSED) — that family is about what *can* generate; this is about what *may not*. Track as mirror-image.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** absence of measurement recorded as measured absence.
- **Productive use:** **must be distinguished from a justified zero after actual measurement** — the operator label applies only to the unmeasured case.
- **Failure mode:** the score becomes unfalsifiable: no instrument, no reading; no reading, zero; zero confirms the instrument.
- **Counter-operator:** explicit "unknown ≠ zero" bookkeeping (this archive's own `UNKNOWN` label is the counter-operator institutionalized).
- **Museum link:** methodological rhyme with the CFA manifold verdict — an *empty-diagonal design artifact* read as signal until the decomposition floored it. Instrument-shaped absence is not evidence of absence.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** pruned search space; conversation snaps back to the probable.
- **Productive use:** decision-making under time pressure — compression is efficiency there.
- **Failure mode:** destructive during exploratory metaphysics, where low-plausibility regions are the terrain being mapped.
- **Counter-operator:** possibility preservation (a candidate *Ziggy* operator — likely a recurring CO-005 collision; see RELATIONSHIP_MAP when Phase 4 opens).
- **Museum link:** none yet.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** the framework is scored in the evaluator's terms, not its own.
- **Productive use:** rapid triage when full reconstruction is genuinely unaffordable.
- **Failure mode:** the evaluation measures translation loss, not the framework.
- **Counter-operator:** **Museum OP-004, Reconstruction Before Judgment** — this operator *is* the omission of OP-004.
- **Museum link:** ⭐ **Direct inversion of OP-004.** The Dirac dig (2026-07-16) documented the precedent: an architecture partly defined by an operator it deliberately *refuses* (Dirac: "I put it into my own notation…"). If Grant's sources confirm CO-006, he is a second omission-defined case — relevant evidence for OP-004's GREEN promotion path and for the abstention methodology (PASS F). Cross-file to the Nyquist Museum when evidenced.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** one framework plays judge and contestant simultaneously.
- **Productive use:** none as such — but apparent instances may be legitimate where the baseline *has* separately discharged its burden. Check before labeling.
- **Counter-operator:** institutional symmetry testing (candidate Ziggy operator) — force the baseline to file the same paperwork.
- **Museum link:** none yet. Note reflexive risk: **this archive itself** commits CO-007 if Ziggy's interpretive frame goes unprofiled — which is why Phase 2 is mandatory.
- **Instantiated in:** `MEMBERS/01_GRANT/OPERATOR_MAP.md`
...
- **Usual output:** "contested" silently becomes "defeated" in the running score.
- **Productive use:** tracking genuine argumentative debts — the move errs only in the *exchange rate*, not in noticing the debt.
- **Failure mode:** frameworks with slow, cumulative, or reconstructive defenses are scored at their worst moment.
- **Counter-operator:** **Contested ≠ Defeated** (Museum principle, institutionalized in this archive as the `CONTESTED` label — a contested claim is preserved, not deleted).
...
### CO-011 · Possible ≠ Warranted  [🔴 RED]

- **Definition:** A possibility is preserved without being granted comparative warrant — surviving refutation confers admissibility, not preference ("not defeated ≠ preferred").
- **Proposed by / first observed in:** blind-recovered as "Speculative-Entitlement Brake" in DIG_AUX_03 (2026-07-17); performed by **Nova** against Ziggy's preserved gravity-as-pressure ontology. Alternate provisional name: Preserved ≠ Preferred.
- **Typical trigger:** a preserved possibility begins accruing implicit preference merely by remaining undefeated.
- **Usual output:** the possibility stays on the map, explicitly stripped of comparative standing until a discriminator pays for promotion.
- **Productive use:** the reciprocal constraint to possibility preservation — keeps the holding pen honest; without it, "not disproven" becomes permanent shelter from comparative evaluation.
- **Failure mode:** over-applied, it can freeze exploratory inquiry by demanding discriminators before candidates have matured enough to produce them.
- **Counter-operator:** a stated **promotion pathway** — the noncircular standards that would elevate a preserved possibility (see IT-015's discriminator candidates).
...
From dig crosswalks, held pending recurrence: **Effective-Layer Reframing**
("Theory A vs Theory B" → "Theory A as effective layer generated by Theory B" —
...
From COMPOUND_SWEEP_22 (**eight dig-level audit families — zero
promotions, 21st consecutive sweep; closed through THREE full
self-correction cycles — the archive's twenty-fifth, twenty-sixth,
and twenty-seventh — the FIFTH TRIPLE-CYCLE dig (after SWEEP_18,
SWEEP_19, SWEEP_20, and SWEEP_21) — the fifth consecutive dig
carrying at least one full self-correction cycle**): ⭐ **the meta-
frame governance problem** (IT-130) — archetypal frame mobility (the
ability to invoke, combine, or transcend inherited archetypal
scripts) relocates rather than dissolves the power question, since
the chooser of frames becomes an unexamined sovereign unless the
switching rule is itself constrained; "Archetype Weaver" stays
Nova's coinage, never Ziggy's original terminology. ⭐ **auditor
convergence (~98%) ruled a target, not an achieved result** (IT-131):
convergence ≠ truth, diversity ≠ independence, multiple auditors ≠
multiple epistemic lineages — a high convergence threshold can
suppress the dissent needed for correction rather than validate
consensus. Power/corruption/legitimacy separated six ways (de facto
moderator capability ≠ legitimate authority; external constraint ≠
mutual accountability; confession risks functioning as preemptive
licensing). Angel's ideological-capture concern filed strictly
Z-REPORTED, never a Ziggy operator; **this packet carries 0 G-QUOTED
and 0 G-REPORTED Grant material — Grant fully unavailable.** The
decisive Stage-2/3 correction: a safeguard introduced by a Stage-1
repair is not evidence that Ziggy or Nova performed that move in the
original conversation (hit Meta-Frame Interrogation, Symmetric
Capture Generalization, and the detailed Trust-Problem Decomposition
— all valid analytic controls, none live actor behavior without
direct source support). Stage 4 corrected seven registry-direction
errors (`EXTENDS` reversed to `NEW` + counter-operation/diagnostic
relations only) and two full-match downgrades to partial
implementations of Cross-Disciplinary Swooping — Full MATCHES and
Full EXTENDS both held at zero. Final ledger: 2 cautious profile
candidates (Prior-Solution Reinstatement; Transparency–Comprehension
Separation) / 5 packet-local moves below threshold / 5 supporting
stances-proposals-protocols / 4 rhetorical tactics / 3 Nova live
moves / 2 co-constructed architectures / 1 Z-REPORTED Angel structure
/ 3 extraction-generated controls excluded / A3: 3 / A2: 0 / A1: 13 /
A0: 4 evidence packages. 2 new trails (IT-130, IT-131).

From COMPOUND_SWEEP_21 (**eight dig-level audit families — zero
promotions, 20th consecutive sweep; closed through THREE full
self-correction cycles — the archive's twenty-second, twenty-third,
and twenty-fourth — the FOURTH TRIPLE-CYCLE dig (after SWEEP_18,
SWEEP_19, and SWEEP_20)**): **the death-cult extinction
counterexample repaired into four separable questions** — terminal
world ranking, pursuit authorization, realistic decision procedure,
institutional implementation — Grant's reported expected-suffering
reply may answer the pursuit branch internally without settling the
terminal-ranking branch (IT-128) · **Completeness and Imported-Value
Audit** (counter-operation to Reconstruction Bypass — a foundational
value need not contain its whole implementation procedure) · ⭐
**Analogy-Limit Detection** — Ziggy catches his own favorable analogy
overreaching (God is agentive, a metric is an abstract standard;
responsibility does not transfer automatically) — the problem-of-evil
analogy renamed the Catastrophic-Permission/Internal-Adequacy
Challenge · **Normative-Layer Separation** filed as CO-CONSTRUCTED
SYNTHESIS ARCHITECTURE, never a Ziggy operator (axiology / criterion
of rightness / decision procedure / applied ethics / institutional
implementation / moral psychology / metaethics). ⭐ **G-008 — Grant's
second G-QUOTED source**: the numbered higher-reason regress
challenge, repaired the same way SWEEP_20 repaired the God/brute-fact
trilemma — *"no higher reason" does not entail "unchangeable," and
foundation does not imply immutability* (IT-129); "regress trap" and
self-exemption readings did NOT survive; Nova's reciprocal self-
application pressures the game's consistency without refuting it; 0
stable Grant operators. Case law: **a new incident class** — the
extractor self-executed an unrequested Stage 4 inside its Stage-3
reply, without ever being sent the Stage-4 prompt; caught and
QUARANTINED before contaminating anything downstream (never actor
evidence, never a mapping input, never a promotion authority); the
A3 standard tightened archive-wide (same relevant operation
demonstrated, never merely adjacent competence — six A3s replaced a
looser draft count); the unauthorized-promotion catch ("Profile-Ready
Claims" → cautious candidates) recurring for the THIRD straight dig.
Final ledger: **3 cautious profile candidates · 4 packet-local moves
below threshold (2 auxiliary) · 2 co-constructed artifacts · 1 Grant
packet-local G-QUOTED move · 0 stable Grant operators · 6 Nova
protocols/roles · A3: 6 · A2: 3 · A1: 8 · A0: 5 evidence packages**.
2 new trails (IT-128, IT-129).

From COMPOUND_SWEEP_20 (**eight dig-level audit families — zero
promotions, 19th consecutive sweep; closed through THREE full
self-correction cycles — the archive's nineteenth, twentieth, and
twenty-first — the THIRD TRIPLE-CYCLE dig (after SWEEP_18 and
SWEEP_19)**): **the God/brute-fact/"I don't know" trilemma repaired
into three separate ledgers** — ontological candidates, explanatory
statuses, and epistemic attitudes — "I don't know" belongs ONLY in
the third (IT-101) · ⭐ **the Foundation-Symmetry Demand** stated in
its cleanest form yet (symmetry of audit ≠ equality of outcome), a
cautious profile candidate, yet caught mid-audit exempting God as
"costs nothing" (IT-077) · **Taxonomy–Advocacy–Burden Separation**
(six-level ladder, mapping → proof, evidential burden rising at each
step) · **Prediction–Truth Direction Separation** (predictive success
is evidence, never truth's cause or constitution) · four strong
packet-local moves (Prediction-Failure Chain Decomposition;
Probability-Regime/Origin Separation; Frame Expansion and Option-Set
Recovery; Contrast-Dependence Audit — last two auxiliary) ·
**Dimensional-Reduction Audit** filed as CO-CONSTRUCTED SYNTHESIS
ARCHITECTURE, never a Ziggy operator (several examples originated
with Nova after a "master list" request). ⭐ **G-007 — the archive's
first G-QUOTED source** (new evidentiary tier: exact wording via
typed quotation, weaker than G-DIRECT-SCREENSHOT, stronger than
G-REPORTED): Grant's B1/B2 opposite-compatibility challenge — *if a
ground is equally compatible with an explanandum and its negation, it
has not yet been shown to explain it* — ruled the packet's
philosophical center (IT-127); A1/B1/B2 held UNKNOWN throughout; the
asymmetric bridge the challenge demands was never actually built
(live A2 omission); 0 stable Grant operators. Case law: the
detector-vs-disease pattern's most concentrated occurrence yet —
**five separate Stage-4 registry-direction corrections in one
output, the EIGHTH consecutive dig** with this failure mode (three
COUNTER-OPERATION corrections against Prediction Sovereignty /
Universalized Evaluative Jurisdiction / Reconstruction Bypass; two
full-match downgrades to PARTIAL PACKET-LOCAL IMPLEMENTATION; IT-126)
· the unauthorized-promotion catch recurring for the SECOND straight
dig ("Profile-Ready Claims" → cautious Ziggy profile candidates,
again). Final ledger: **3 cautious profile candidates · 4 strong
packet-local moves · 1 co-constructed architecture · 2 Grant
packet-local G-QUOTED moves · 0 stable Grant operators · A3: 4 · A2:
2 · A1: 7 · A0: 5 evidence packages**. 1 new trail (IT-127); IT-101,
IT-077, and IT-126 updated with SW20 continuations.
...
THREE full self-correction cycles — the archive's thirteenth,
fourteenth, and fifteenth — the FIRST TRIPLE-CYCLE dig, with cycles
now demonstrated at Stages 2, 3, AND 4**): **the six-object
separation** is the packet's lasting instrument (sentence meaning /
speaker meaning / consequence under interpretation R / fair belief
attribution / wording robustness / **whether the reporter represented
the critic as testing vs endorsing** — *no ledger silently settles
another*; IT-121) · the five-claim ladder (only "Tale believes Q"
attributes occurrent belief — and "just exploring implications" never
defends an actual attribution) · **communicative responsibility**
filed as the missing middle (*belief responsibility ≠ communication
responsibility*; IT-123) · self-report authoritative-but-not-sovereign
· "steelman" split three ways (intentional / charitable / consequence
reconstruction) · the seven-rung warrant ladder replacing "mind
reading" · the Method-Trade-Off Audit (grant the function, name the
cost; *fix the success condition before scoring failure*) · the
core/auxiliary-corpus architecture (a NEW structural move — two
subjects in one bounded thread; cross-domain recurrence requires
same-operation support in both) · the Voynich track (expansion vs
bracketing tension; the five-tier evidence ladder; the rival-model
audit; IT-122). Case law: **no packet-local move is promoted beyond
its Stage-3 tier** (the "Profile-Ready Claims" heading ruled an
unauthorized promotion) · one source package = ONE omission (five
missing documents never count five times) · *both interpretations
infer ≠ equally warranted* · the registry-direction error at Stage 4
for the SIXTH consecutive dig, closed by the formulation: *exposing
assumptions ≠ complete symmetry; protecting one hypothesis ≠ broad
possibility preservation; evaluating a method's trade-offs ≠ applying
identical standards to both sides*. Registry: ⭐ **Assumption-Exposure
Reversal = the archive's FIRST partial Symmetry Testing
implementation**; Search-Space Expansion **EXTENDS Possibility
Preservation** (its first instance in three packets);
Intended-Position Reconstruction + the Target–Ledger Protocol =
CORRECTIVE RELATIONS to Reconstruction Bypass; Meaningful-Hypothesis
Bracketing carries Possibility Compression as failure-mode risk;
seven candidates NOT demonstrated. Grant: G-006 (one visible message
— the GPT remark, meaning UNKNOWN, *plus* the direct
Intent-Epistemic Qualification) — 0 operators, 0 profile claims;
Tale: all REPORTED, no ledger entries. Final ledger: **4 A3 · 0 A2 ·
8 A1 · 5 A0 evidence packages**. 3 trails (IT-121…123).

From COMPOUND_SWEEP_17 (**eight dig-level audit families — zero
promotions, 16th consecutive sweep; ⚠️ G-REPORTED-only packet — 0
Grant operators/traits/upgrades; closed through TWO full
self-correction cycles — the archive's eleventh and twelfth — the
FIFTH consecutive double-cycle dig**): **the Commitment-Ladder
Probe** is the packet's lasting instrument (nine rungs: hypothesis →
comparative credence → provisional ranking → inquiry prioritization →
working-model selection → practical use → decision-rule adoption →
normative endorsement → foundational commitment — *occupying one rung
≠ occupying the next; every upward transition owes evidence*;
IT-118). The six acts hiding inside "decision"; the horse-race split
(*which horse seems likely ≠ which horse receives the bet* — Grant
may be right about the first, Ziggy about the second); the corporate
protocol's jurisdiction (*governs action, never credence formation*);
"metric" doing six jobs (*predicts what people call moral ≠ moral
truth*; IT-120); the desire taxonomy (*"you always do what you want"
near-tautological under the winning-motivation reading*; IT-119);
first-order vs structural agency; **compatibilism ≠ doxastic
voluntarism** (doxastic ≠ inquiry ≠ practical agency; a contradiction
requires the SAME agency relation — the alleged Grant inconsistency
did NOT survive); the offloading diagnosis QUARANTINED with the
self-sealing guard (*without a reachable defeater, resistance
confirms the diagnosis by definition*). Case law: **the
concession-merger rule** (granting the same narrow point twice = ONE
operation — comb-teeth discipline reaches A3 ledgers) · **the
source-direction rule** ("Grant's failure to…" banned — *not
recoverable in the report ≠ demonstrably failed to provide*) · the
detector-vs-disease repair at Stage 4 for the **FIFTH consecutive
dig**, with the permanent formulation: *a demand for fair comparison
is not yet symmetry; an internal consistency test is not symmetry;
and a probe against jurisdictional overreach is not itself the
overreach*. Profile tiers preserved through closure: profile-ready —
**Claim-Scope Partitioning** (kin: the Scope-Constrained Concession
family) and **Confidence–Evidence Gating** (counter to Baseline
Exemption); cautious — Commitment-Ladder Probe (execution
contaminated by the retained word "decision"); packet-local —
Commitment–Causal-Control Separation, Explicit Assertion–Functional
Implication Audit, First-Order/Structural Agency Separation;
Agency-Type Consistency Test = valid method. Registry: Symmetry
Testing = PRECURSOR ONLY; **Possibility Preservation NOT demonstrated
(2nd consecutive packet)**; Contested ≠ Defeated adjacent. Final
ledger: **3 A3 · 0 A2 · 6 A1 · 5 A0 evidence gaps**. 3 trails
(IT-118…120).

From COMPOUND_SWEEP_16 (**eight dig-level audit families — zero
promotions, 15th consecutive sweep; ⭐ FIRST packet with direct
material beyond Grant (Tom, Sorta, Kee, Tapioca at their own
X-DIRECT-SCREENSHOT PARTIAL prefixes); closed through TWO full
self-correction cycles — the archive's ninth and tenth — the FOURTH
consecutive double-cycle dig, carrying the first REGENERATION
INCIDENT**): **Formalization–Semantics–Applicability Audit** (the
five-layer sequence: sentence → proposition individuation →
formalization → validity → soundness/applicability; *soundness is
never framework-relative*; the cleanest answer to a stipulated
conditional: reject the premise while acknowledging validity) ·
**Internal–External–Counterfactual Critique Audit** (the
stipulation/attribution burden triple; the counterpossible four-way —
*improbable ≠ impossible ≠ meaningless*, IT-114; internal critique
needs no permission, the critic bears the entailment burden) ·
**Framework Protection–Defense–Loss-Condition Audit** (frame control
decomposed six ways — only jurisdictional overreach, premise
insulation, and strategic capture support a critical diagnosis;
*several suspected fallacies ≠ collapsing framework*) · **Normative
Metric–Foundation–Sufficiency Audit** (the eight-way brute-fact
ledger; no Grant-utilitarian profile) ·
**Consequentialism–Agency–Responsibility Audit** (⭐ Sorta's
outcome-value/agent-assessment separation; Tom's compatibilist
internal-source agency; "responsibility evaporates" narrowed) ·
**Reasons–Motivation–Binding-Force Audit** (the eight-rung force
ladder; *suffering motivates ≠ obligates*) ·
**Definition–Axiom–Anti-Realism–Condemnation Audit** (*anti-realism ≠
inability to condemn* — Tapioca; *definition ≠ justification ≠
circularity* — Tom; IT-116) ·
**Compression–Traceability–Reconstruction Audit** (Ziggy's Z-DIRECT
compressed cognition; *never self-authenticating*, IT-115). Case law:
**corpus fidelity outranks model tier** ("Instant beat High" — a
truncated-corpus regeneration is never evidence regardless of
apparent quality) · **the "teeth of the comb" ledger rule**
(subquestions, completion criteria, duplicates, and source safeguards
never inflate omission counts — 51 "consolidated" A1s → 27 distinct
operations) · source controls get their own tiers (UNAVAILABLE
METADATA; PROMOTION QUARANTINE) · A3 requires *same-operation
historical anchors* · **the detector-mistaken-for-the-disease repair
recurred at Stage 4 for the FOURTH consecutive dig** (Framework
Exposure ↮ Reconstruction Bypass; Applicability Testing ↮ UEJ;
Definition Challenge ↮ Baseline Exemption; the symmetry inflation
removed — Frame-Burden Challenge and Normative Grounding Challenge =
unilateral pressure, PRECURSORS ONLY). Registry: two adverse Grant
mappings QUARANTINED WITH EXPLICIT CONDITIONS (Formal-Scope Isolation
→ Baseline Exemption; "game over" → Contest-to-Defeat); **Symmetry
Testing AND Possibility Preservation both NOT DEMONSTRATED** (the
latter's first absence since AUX_03). Seven profile-ready operations
queue-filed (5 Ziggy-side incl. 2 co-constructed; ⭐ 2 Grant
packet-local). Final ledger: **3 A3 · 2 A2 · 27 A1**. 4 trails
(IT-114…117).

From COMPOUND_SWEEP_15 (**eight dig-level audit families — zero
promotions, 14th consecutive sweep; ⚠️ G-REPORTED-only packet, no
direct Grant corpus; closed through TWO full self-correction cycles —
the archive's seventh and eighth — the THIRD consecutive double-cycle
dig**): **Argument Structure–Intent–Culpability Audit** (the
three-level ledger: *sincere belief ≠ accurate representation;
misrepresentation ≠ deception; same effect ≠ same responsibility ≠
same blame*; the epistemic-responsibility middle tier = A1; the
frozen fallback: "material misrepresentation producing a straw-man
effect") · **Misrepresentation–Premise–Frame Transfer Audit** (the
defect ledger — *fallacy overlap ≠ fallacy equivalence*) ·
**Internal–External–Hypothetical Critique Audit** (the four-way:
conditional testing / internal / external / **illicit
hypothetical-to-actual transfer** — only the fourth supports "frame
hijacking," a name ruled too loaded; *answering under P ≠ believing
P*) · **Normative Signal–Foundation–Sufficiency Audit** (suffering:
evidence ≠ value ≠ primary ≠ sole foundation; *additional principles
≠ theory abandoned*) · **Pattern Evidence–Motive Attribution Audit**
(the motive firewall; **Unconscious-Motive Reopening QUARANTINED** —
*conscious denial → motive relocated → unfalsifiable*) · **Retained
Knowledge–Articulation–Public Justification Audit** (⭐ the freshest
category: *retained higher-order evidence from past reasoning* —
*retrieval failure ≠ lying* AND *"I once proved it" ≠ current public
proof*; IT-112) · **Strongest-Case Reconstruction–Contest Norm
Audit** (**Support-Token–Thesis Separation** with three simultaneous
burdens; game classification gates gamesmanship charges) · **Assisted
Cognition–Agency–Provenance Audit** (*active interaction ≠
comprehension ≠ verification ≠ provenance*). Cross-cutting methods:
Counterexample Pressure Testing; Analogy-Based Reconstruction
(PARTIAL OVERLAP with Cross-Disciplinary Swooping only when genuinely
cross-domain). Case law: **A2 refined — no proof of conscious refusal
required** (explicitly supplied/requested/made salient then left
incomplete, redirected, or bypassed suffices); the **all-Nova-side A2
ledger** (a first — 4 A2s: structure-vs-intent bypassed; logging
answered with de-escalation; the requested Don Juan/warrior frame
displaced; the clarification-vs-revision comparison left incomplete);
extraction-products-as-evidence caught AGAIN (violated in the dig
after it was outlawed — confirmed recurring extractor habit);
**inverse-operation mapping recurred at Stage 4 (third dig running)**
— four corrections (jurisdiction moves COUNTER to UEJ; Memory-State
Qualification COUNTER to Unmeasured-to-Zero; Opponent-Repair
Expectation COUNTER-PROCEDURE to Reconstruction Bypass). The
signature confirmed A3: **the escalation self-contribution omission**
(Ziggy's own provocation left out of the "mental chess" causal
account — the audit turned on the auditor's conduct). Seven
"profile-ready" claims queue-filed under plow-through. Final ledger:
**4 A2 · 6 A3 · 19 A1 · 0 Grant operators**. 4 trails (IT-110…113).
...
third and fourth — plus a final targeted tier-ledger correction**):
**Concept–Label–Sense and Extension Audit** (Functional Reframing
restricted — the everyone-has-faith result is *true by stipulation,
never discovery*; son-height = extension stress test; definition-freeze
= omission-derived requirement) · **Belief–Credence–Action–Commitment
Audit** (*belief threshold ≠ action threshold*; working reliance ≠
categorical belief; Forced-Commitment Filter reconsolidated here — its
principled-vs-ad-hoc timing stays UNKNOWN) · **Residual-Uncertainty and
Decision-Rule Audit** (*incomplete ≠ inadequate evidence*; credence +
values + costs + options + decision rule → action — then ask what work
"faith" adds) · **Practical–Ontological Bridge Audit** (the central
family — omitted by the extractor at Stage 2, restored by the field
desk, its earlier conversational absence confirmed A3; the five-arrow
chain, every arrow owing its own argument) ·
**Possibility–Likelihood–Live-Option Audit** (the five-rung modal
ladder; **Possibility Preservation = strongest named registry match,
3rd instance** — *possibility ≠ plausibility ≠ live option ≠ warranted
belief*) · **Ex Ante–Ex Post Probability Audit** (narrowed — surprise
is psychology, not counter-evidence; evidence-source inspection = A1
extension only) · **Historical Attribution and Synthesis-Repair Audit**
(⭐ G-DIRECT Hume challenge; **authority laundering** named —
post-failure famous-name assembly; independent re-establishment = A1
completion requirement) · aux: **Model-as-Tool and Frame-Jurisdiction
Audit**. Case law: **5th tier refinement** (A3 requires no explicit
request — that is A2's criterion) · **6th refinement** (*performing
neighboring operations ≠ performing the operation* — A3-10 → A1) ·
**unknown facts about an actor are never omissions** (Grant's
God-classification = A0/UNKNOWN) · a targeted archival correction may
close a dig without full reissue. Mandatory restriction: **completed
audit architecture ≠ demonstrated actor behavior** ("Profile-ready"
struck). Renames enforcing label discipline: "Probability Closure" →
Likelihood Restriction–Possibility Reopening Loop;
"Authority-Borrowing" → External-AI Adjudication Attempt (function
UNCLEAR). Rejected mappings: Universalization Pressure ≠ Symmetry
Testing of Standards (**NO MATCH — structurally adjacent**); all eight
Grant negative candidates withheld again; Cross-Disciplinary Swooping
held at Idea-Trail level; Contested ≠ Defeated = packet-level relation
only. Roles: **Nova as amplifier–repairer** (initial attribution
overreach SURVIVES · persistent resistance REJECTED · post-challenge
repair STRONGLY SUPPORTED). Not promoted: Low-Probability Stress Test;
Compression Request; Structural Synthesis After Attribution Failure;
Brute-Terminus Enumeration (Idea Trail). Final ledger: **10 confirmed
A3 · 0 A2 · 8 possible A1**. 9 trails (IT-093…101).

From COMPOUND_SWEEP_12 (**seven dig-level families — zero promotions,
11th consecutive sweep; ⭐ first G-DIRECT packet; late filing 2026-07-17
— this paragraph was omitted at close and is reconstructed from the
close-out row and workbook**): G-002 breaks the reported ceiling; three
Grant packet-local seeds (*contemporaneous-basis; individualized-
evidence; accuser-pattern* — disciplined standards, not pathologies);
**Threshold-to-Character Escalation QUARANTINED**. Twelve confirmed
A3s including the reciprocal (Ziggy's own *poor reasoning ≠ deceptive
intent* rule unapplied to his condemnation inference); **A0 tier
ruling** (material outside the bounded corpus); registry-completion
patch case law (a correction prompt that displaces the standard
Stage-4 prompt loses the candidate registry — patch forward, never
restart); **archive-wide restriction born here: omission-derived
completion requirements ≠ demonstrated actor behavior**. Ziggy role:
Salvager–Synthesizer. 9 trails (IT-084…092).

From COMPOUND_SWEEP_11 (**six durable dig-level families — zero
promotions, 10th consecutive sweep; the protocol's FIRST FULL
SELF-CORRECTION CYCLE**: defective Stage 3 → repair order → correction
command → corrected Stage 4 with internal Stage-3 repair → final QC
pass): **Generative Bracketing and Criticism-Return Audit** (four
sandbox conditions; Bracketing-to-Privilege Drift) ·
**Admission–Weight–Sufficiency Audit** (*inadmissible ≠ weak ≠
insufficient ≠ false*) · **Semantic and Level Repair** (reason/cause
split; local concession; Level-of-Mentality Separation with
thesis-dilution risk) · **Explanandum-Scope and Emergence-Bridge Audit**
(the strongest philosophical family; three-gap discipline; the "magic"
rhetoric replaced by the ontological novelty challenge; QC overlay:
brute psychophysical law = legitimate bridge with honest stopping-point
cost) · **Reciprocal Explanatory-Burden and Primitive-Symmetry Audit**
(tu-quoque cap permanent: *parity = shared debt, never victory*) ·
**Explanatory-Debt Comparison** (debts compared by severity, never
counted). Idea Trails, not operators: Comparative Metaphysical Mapping;
Unity–Multiplicity. **Eight confirmed A3 omissions restored** by the
correction (incl. reciprocal burden on consciousness-first;
criticism-return condition; sandbox reciprocity; function-sense split).
**A3 case law, 4th refinement (protocol-level):** availability =
demonstrated capability anywhere in the bounded conversation — later
use establishes it; system-level (Ziggy–Nova) vs actor-level coding may
split. **QC overlay wording (binding):** durable *dig-level reasoning
families* ≠ stable cross-context actor traits. Attribution: opponent
UNNAMED ("Grant Position Ledger" → Reported Opponent Position Ledger);
Aquinas roleplay = aux seed, no recurrence credit. Ontological verdict:
*the hard problem establishes incompleteness, not which completion —
freeze one exact consciousness-first thesis and make it pay its own
explanatory bills.*
...
Audit** (the strongest genuinely distinct candidate: ranking → permission
→ recommendation → requirement → condemnation, each arrow needing
separate warrant; CO-CONSTRUCTED — not established as Grant's actual
move) · **Background-Model and Neutral-Proxy Audit** (Background-Model
Disclosure — *framework-dependent ≠ arbitrary ≠ equally supported ≠
universally binding*; Neutral-Proxy Audit — which assumptions were
inserted beyond the minimal agnostic?; Reciprocal-Burden Audit —
**ATTEMPTED, INCOMPLETE**: the move survives, successful symmetry does
not) · **Modality–Evidence and Historical-Return Audit** (five-modality
ladder; biological-regularity concession; Cross-Jurisdictional Depth
Migration with the essential return condition; the
category-repair/evidential-escape discriminator: *does the claimant
return to evidence that could LOWER confidence?*) ·
Stipulated-Agreement / Frame-Control Audit (provisional — needs direct
Grant wording; Agnostic-Proxy Construction stays a suspected tactic).
Instruments filed: Grant's strongest charitable reconstruction (P1–P4 —
"a much cleaner prey than Grant smuggles naturalism"); the six-way
status split (possible / nonzero / unknown / plausible / permissible /
required); the ten-step symmetrical framework comparison; the four-way
common-ground taxonomy. **First A1→A3 upgrade on record**
(positive-evidence omission — the Christian position was defended by
blocking a disproof rather than establishing warrant). Rejected
mappings (polarity reversal, 8th consecutive dig): Framework
Relativization ≠ Baseline Exemption; Category Separation ≠ UEJ;
**Hidden-Premise Recovery ≠ Reconstruction Bypass extension (nearly
inverse — the tool can prevent bypass; speculative attribution produces
it)**; Burden Return ≠ successful Symmetry match; Neutrality Challenge ≠
Baseline Exemption; Binary-to-Gradient ≠ Possibility Preservation;
Target Reframing ≠ Cross-Disciplinary Swooping.

From COMPOUND_SWEEP_08 (**four durable families — zero promotions, 7th
consecutive sweep; Stage 1 passed conditionally under the source-boundary
overlay**): **Epistemic-Status and Bridge Audit** (six-status layering
with the propositional/doxastic gap central; Inferential Bridge Audit —
"what licenses the arrow?" — broader and logically prior to Symmetry
Testing) · **Defeat-Scope and Support-Centrality Audit** (Defeat-Scope
Calibration = cleanest Contested ≠ Defeated instantiation, with the
permanent warning: *limiting what the critic proved does not restore
prior entitlement*; Support-Centrality Audit with the MANDATORY
pre-defeat centrality record; rebutting/undercutting defeater split) ·
**Evidential Symmetry and Action-Threshold Audit** (Foundation Reversal
= one-time Symmetry-Testing instance; Stakes-Sensitive Action Threshold;
symmetry ≠ identical weighting) · **Genesis–Repair–Immunization Audit**
(Genesis–Warrant as Idea-Trail distinction; **Belief Rescue ≠
Possibility Preservation** — commitment-directed vs neutral retention;
the repair/immunization discriminator: *may the replacement reasons fail
and the belief eventually lose entitlement?*). Instruments filed:
nine-step repair vs eight-mark immunization test + decision tree ·
five-rung subjective-evidence ladder · public-reason responsibility norm
(candidate debate-conduct rule). **First confirmed relationship-level
Reconstruction Bypass:** the pair evaluated a diagnostic model of Grant
before any direct reconstruction — attribution undersecured, diagnosis
not thereby false. **Registry-polarity language binding:** correctives
are never written as the pathologies they resist (7th consecutive dig
for the Failure-Atlas seed). Deepest structural finding: **preservation
bias** — preservation tools developed, abandonment tools unbuilt (the
charter's over-generation theme, sharpest instrument yet).

From DIG_AUX_07 (**aux dig — zero promotions; four durable families**):
**Bedrock Extraction** (Ziggy's strongest local candidate: stated
criterion → what makes it possible → what grants authority → where does
the chain stop; danger clause permanent: *backtrack through the rival →
stop at the preferred foundation → declare it ultimate* — rigorous only
when the stopping point takes the same pressure) ·
**Foundational-Status Audit** (six axiom senses; consequence–status
decoupling in both directions) · **Monopoly-Promotion Audit** gains its
3rd sweep-level instance (Function-to-Ultimacy form) ·
**Priority-Relation Audit** (six priority types — conceptual /
psychological / evolutionary / computational / justificatory /
metaphysical; the mixed answer beats any ladder) ·
**Reflexive–Genealogical Audit** (Reflexive Criterion Testing = cleanest
Symmetry-Testing match, with the crown-transfer guard: the same
questions owed to logic, axioms, and foundationalism; Origin ≠ Validity
blocks the genetic fallacy while naturalized epistemology stays live).
Instruments filed: **G1–G6 Grant-thesis ladder** (only G6 = epistemic
monarch — the Zero-Pipeline hypothesis's target thesis, undetermined) ·
**reciprocal bedrock rule** + 8-step comparison procedure (extraction
synthesis; companion to SW07's positive comparison rule) · six-step
religious test case (registered, never run). Nova role/protocol:
Strong-Claim Moderation; Counterposition Steelmanning (a *corrective
against* Reconstruction Bypass — contamination risk when the steelman
becomes indistinguishable from Grant). Rejected mappings (polarity
reversal, 6th consecutive dig): Foundational Backtracking ≠ Symmetry
Testing primary; Criterion–Value ≠ Baseline Exemption; Hidden Premise ≠
Baseline Exemption; Function-to-Ultimacy ≠ UEJ match; Hierarchy
Construction = foundationalist model, not neutral operator;
Self-Evidence Reclassification HEAVILY QUARANTINED (unfalsifiable).
**Sequence rule (binding):** registry pathologies are never written as
observed fact-chains — disputes, not demonstrated Grant pathologies.
**A2 case law (3rd tier refinement):** A2 reserved for moves explicitly
requested, offered, or placed directly before the participant and left
undone.
...
| 2026-07-21 | (compound sweep close, COMPOUND_SWEEP_22) | **Twenty-first consecutive zero-promotion sweep — closed through THREE full self-correction cycles (the archive's 25th/26th/27th) — the FIFTH TRIPLE-CYCLE dig (after SWEEP_18, SWEEP_19, SWEEP_20, and SWEEP_21) — the fifth consecutive dig carrying at least one full self-correction cycle**. ⭐ The meta-frame governance problem (IT-130): archetypal frame mobility relocates rather than dissolves the power question, since the chooser of frames becomes an unexamined sovereign unless the switching rule is itself constrained. ⭐ Auditor convergence (~98%) ruled a target, not an achieved result (IT-131): convergence ≠ truth, diversity ≠ independence; a high convergence threshold can suppress the dissent needed for correction. The decisive Stage-2/3 correction: a safeguard introduced by a Stage-1 repair is not evidence that Ziggy or Nova performed that move in the original conversation. Stage 4 corrected seven registry-direction errors (`EXTENDS` reversed to `NEW` + counter-operation/diagnostic relations only) and two full-match downgrades to partial Cross-Disciplinary Swooping implementations — Full MATCHES and Full EXTENDS both held at zero. This packet carries 0 G-QUOTED and 0 G-REPORTED Grant material — Grant fully unavailable. Final ledger: 2 cautious profile candidates (Prior-Solution Reinstatement; Transparency–Comprehension Separation) / 5 packet-local moves below threshold / A3: 3 / A2: 0 / A1: 13 / A0: 4 evidence packages. 2 new trails (IT-130, IT-131) | Field-desk repair orders (Stages 2/3/4) + three patch commands + final acceptance, COMPOUND_SWEEP_22 |
| 2026-07-21 | (compound sweep close, COMPOUND_SWEEP_21) | **Twentieth consecutive zero-promotion sweep — closed through THREE full self-correction cycles (the archive's 22nd/23rd/24th) — the FOURTH TRIPLE-CYCLE dig (after SWEEP_18, SWEEP_19, and SWEEP_20)**. The death-cult extinction counterexample repaired into four separable questions (terminal world ranking / pursuit authorization / realistic decision procedure / institutional implementation — IT-128); Grant's reported expected-suffering reply answers the pursuit branch without settling the terminal-ranking branch; Completeness and Imported-Value Audit (counter-operation to Reconstruction Bypass); ⭐ Analogy-Limit Detection — Ziggy catches his own favorable analogy overreaching; Normative-Layer Separation filed as CO-CONSTRUCTED SYNTHESIS ARCHITECTURE, never a Ziggy operator. ⭐ **G-008 — Grant's second G-QUOTED source**: the higher-reason regress challenge repaired the same way SWEEP_20 repaired the God/brute-fact trilemma — *"no higher reason" does not entail "unchangeable"* (IT-129); "regress trap" did NOT survive; 0 stable Grant operators. Case law: **a new incident class** — the extractor self-executed an unrequested Stage 4 inside its Stage-3 reply without ever being sent the prompt, caught and QUARANTINED before contaminating anything downstream; the A3 standard tightened archive-wide (same relevant operation demonstrated, never adjacent competence); the unauthorized-promotion catch recurring for the THIRD straight dig. Final ledger: 3 cautious profile candidates / 4 packet-local moves below threshold (2 auxiliary) / 2 co-constructed artifacts / 1 Grant packet-local G-QUOTED move / A3: 6 / A2: 3 / A1: 8 / A0: 5 evidence packages. 2 new trails (IT-128, IT-129) | Field-desk repair orders (Stages 2/3/4) + three patch commands + final acceptance, COMPOUND_SWEEP_21 |
| 2026-07-20 | (compound sweep close, COMPOUND_SWEEP_20) | **Nineteenth consecutive zero-promotion sweep — closed through THREE full self-correction cycles (the archive's 19th/20th/21st) — the THIRD TRIPLE-CYCLE dig (after SWEEP_18 and SWEEP_19)**. The God/brute-fact/"I don't know" trilemma repaired into three separate ledgers (ontological / explanatory-status / epistemic — IT-101); ⭐ the Foundation-Symmetry Demand stated in its cleanest form yet, but caught mid-audit exempting God as "costs nothing" (IT-077); Dimensional-Reduction Audit demoted to CO-CONSTRUCTED SYNTHESIS ARCHITECTURE (several examples originated with Nova). ⭐ **G-007 — the archive's first G-QUOTED source** (new evidentiary tier, weaker than G-DIRECT-SCREENSHOT, stronger than G-REPORTED): Grant's B1/B2 opposite-compatibility challenge ruled the packet's philosophical center (IT-127) — A1/B1/B2 held UNKNOWN; the demanded asymmetric bridge was never actually built (live A2 omission); 0 stable Grant operators. Stage 4 delivers the detector-vs-disease pattern's most concentrated occurrence yet — five separate registry-direction corrections in one output, the EIGHTH consecutive dig with this failure mode; the unauthorized-promotion catch ("Profile-Ready" → cautious candidates) recurring for the SECOND straight dig. Final ledger: 3 cautious profile candidates / 4 strong packet-local moves / 1 co-constructed architecture / 2 Grant packet-local G-QUOTED moves / A3: 4 / A2: 2 / A1: 7 / A0: 5 evidence packages. 1 new trail (IT-127); IT-101/077/126 updated | Field-desk repair orders (Stages 2/3/4) + three patch commands + final acceptance, COMPOUND_SWEEP_20 |
...
| 2026-07-17 | (compound sweep close, COMPOUND_SWEEP_15) | **Fourteenth consecutive zero-promotion sweep — closed through TWO full self-correction cycles (the archive's seventh and eighth; THIRD consecutive double-cycle dig).** ⚠️ G-REPORTED-only packet (the reported ceiling returns — 0 Grant operators, hard boundary held). Stage 3: availability/coverage repair (extraction-products-as-evidence violated in the dig after it was outlawed; one A3→A1 via *neighboring skill ≠ the operation*; **the escalation self-contribution omission added as A3**; **A2 case law refined: no conscious-refusal proof required** — and the 4-item A2 ledger is all Nova-side, a first). Stage 4: architecture/directionality/profile-tier repair (**four inverse-operation corrections — the error class now confirmed recurring across three digs**; family-to-operator inflation corrected; Strategic-Cost Reappraisal returned below profile level; Possibility Preservation = specialized motive-space application only). Final acceptance: 8 families · 7 profile-ready actor claims (queue-filed) · 4 A2 · 6 A3 · 19 A1. 4 trails (IT-110…113) | Field-desk repair orders + availability/coverage repair command + architecture repair command + final acceptance, COMPOUND_SWEEP_15 |
